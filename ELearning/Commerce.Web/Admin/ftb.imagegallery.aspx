...
// Messages
private string NoFileMessage = Resources.Admin.NoFileSelected;
private string UploadSuccessMessage = Resources.Admin.UploadedSucess;
    private string NoImagesMessage = Resources.Admin.NoImages;
    private string NoFolderSpecifiedMessage = Resources.Admin.NoFolder;
    private string NoFileToDeleteMessage = Resources.Admin.NoFileToDelete;
    private string InvalidFileTypeMessage = Resources.Admin.InvalidFileType;
			...
			if (UploadFile.PostedFile.FileName.Trim() != "") {
				//if (IsValidFileType(UploadFile.PostedFile.FileName)) {
					try {
                        ...
                        ResultsMessage.Text = Resources.Admin.Error;
					}
				//} else {
...
public void DeleteImage_OnClick(object sender, EventArgs e) {
	if (FileToDelete.Value != "" && FileToDelete.Value != "undefined") {
		try {
			...
			ResultsMessage.Text = string.Format(Resources.Admin.Deleted, FileToDelete.Value);
		} catch(Exception ex) {
            ResultsMessage.Text = Resources.Admin.Error;
		}
	} else {
...
public void DisplayImages() {
	...
	if ( (FilesArray == null || FilesArray.Length == 0) && (DirectoriesArray == null || DirectoriesArray.Length == 0) ) {
		...
		if (CurrentImagesFolder.Value != RootImagesFolder.Value) {
			...
			myTitleHolder.Controls.Add(new LiteralControl(Resources.Admin.Up));
