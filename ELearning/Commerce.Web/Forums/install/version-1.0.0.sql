...
--Get Id by UserName
if exists (select 1 from sysobjects where id = object_id(N'yaf_user_getIdByUserName') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	drop procedure yaf_user_getIdByUserName
GO

CREATE procedure [dbo].[yaf_user_getIdByUserName](@BoardID int,@Name nvarchar(50)) as
begin
	declare @UserID int

	-- Try correct board first
	if exists(select UserID from yaf_User where Name=@Name and BoardID=@BoardID)
	begin
		select UserID from yaf_User where Name=@Name and BoardID=@BoardID
		return
	end

	if not exists(select UserID from yaf_User where Name=@Name and (BoardID=@BoardID or (Flags & 3)=3))
		set @UserID=null
	else
		select 
			@UserID=UserID 
