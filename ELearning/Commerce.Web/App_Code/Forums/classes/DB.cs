...
namespace yaf
{
	...
	public class DB
	{
		...
		{			
			SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["CommerceTemplate"].ConnectionString);
			conn.Open();
			return conn;
		}
		/// <summary>
        ...
        static public int user_getIdByUserName(int boardID, string name)
        {
            using (SqlCommand cmd = new SqlCommand("yaf_user_getIdByUserName"))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add("@BoardID", boardID);
                cmd.Parameters.Add("@Name", name);
        ...
        static public bool user_register(string userName, string password, string email)
        {
            string hashinput = DateTime.Now.ToString() + email.ToString() + pages.register.CreatePassword(20);
            string hash = FormsAuthentication.HashPasswordForStoringInConfigFile(hashinput, "md5");
                ...
                using (SqlTransaction trans = conn.BeginTransaction(m_isoLevel))
                {
                    try
                    {
                        using (SqlCommand cmd = new SqlCommand("yaf_user_save", conn))
                        {
                            ...
                            int UserID = 0;
                            cmd.Parameters.Add("@UserID", UserID);
                            cmd.Parameters.Add("@BoardID", ConfigurationManager.AppSettings["DefaultBoardId"]);
                            cmd.Parameters.Add("@UserName", userName);
                            cmd.Parameters.Add("@Password", FormsAuthentication.HashPasswordForStoringInConfigFile(password.ToString(), "md5"));
                            cmd.Parameters.Add("@Email", email);
                            cmd.Parameters.Add("@Hash", hash);
                            cmd.Parameters.Add("@Location", string.Empty);
                            cmd.Parameters.Add("@HomePage", string.Empty);
                            cmd.Parameters.Add("@TimeZone", ConfigurationManager.AppSettings["DefaultTimeZone"]);
