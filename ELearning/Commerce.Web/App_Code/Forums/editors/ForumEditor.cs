...
namespace yaf.editor
{
	...
	public class TextEditor : ForumEditor
	{
		...
		protected virtual void Editor_Load(object sender,EventArgs e)
		{
			if (this.Visible)
			{
				Page.RegisterClientScriptBlock("yafeditorjs",string.Format("<script language='javascript' src='{0}'></script>",((Control)this).ResolveUrl(ResolveUrl("yafEditor/yafEditor.js"))));
	...
	public class BBCodeEditor : TextEditor
	{
		private void RenderButton(HtmlTextWriter writer,string id,string cmd,string title,string image)
		{
			...
			writer.WriteLine("<img id='{1}_{4}' onload='Button_Load(this)' src='{0}' width='21' height='20' alt='{2}' title='{2}' onclick=\"setStyle('{4}','')\">",((Control)this).ResolveUrl(ResolveUrl(image)),SafeID,title,cmd,id);
		}
...
//				typEditor = Type.GetType(ClassBinStr,true);
				typEditor = System.Web.Compilation.BuildManager.GetType(ClassBinStr,true);
			}
			catch (Exception)
			{
#if DEBUG
